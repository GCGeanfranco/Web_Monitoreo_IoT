...
const char* API_URL_TRANSFORMADOR = "https://webmonitoreoiot-production.up.railway.app/api/lecturas/transformador";
const char* API_URL_CONTROL       = "https://webmonitoreoiot-production.up.railway.app/api/control/estado";
...
void leerComandosControl() {
  if (!wifi_conectado || WiFi.status() != WL_CONNECTED) return;
...
float leerVoltajeRMS() {
  ...
  if (voltaje > 280.0f) voltaje = 280.0f;
  return voltaje;
}

float leerVoltajeEstable() {
  float suma = 0;
  for (int i = 0; i < 3; i++) suma += leerVoltajeRMS();
  return suma / 3.0f;
}
...
int calcularTap(float voltaje, int tap_prev) {
  if (voltaje < V_CRITICO) return -1;

  for (int t = NUM_TAPS; t >= 1; t--) {
    float ub = TAP_CONFIG[t].umbral_bajo;
    float ua = TAP_CONFIG[t].umbral_alto;
    if (t == tap_prev) { ub -= V_HISTERESIS; ua += V_HISTERESIS; }
    if (voltaje >= ub && voltaje < ua) return t;
  }
  return 1;
}
...
void abrirTodosReles() {
  for (int t = 1; t <= NUM_TAPS; t++)
    if (PIN_RELAY[t] >= 0) digitalWrite(PIN_RELAY[t], RELAY_OFF);
}
...
if (wifi_conectado) {
  ...
  if (v_inicial >= V_CRITICO) {
    ...
    if (tap_ini >= 1) conmutarTap(tap_ini);
  } else {
    activarAlarma();
  }
...
if (wifi_conectado) {
  ...
  if ((ahora - g_ultimo_ciclo) >= CICLO_MS) {
    ...
    if (!hay_alarma) gestionarTaps(g_voltaje);
  }
